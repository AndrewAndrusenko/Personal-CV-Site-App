import { Component } from '@angular/core';
import { Subject, Subscription } from 'rxjs';
import { CommService, languages } from '../comm.service';

@Component({
  selector: 'app-typing-animation',
  templateUrl: './typing-animation.component.html',
  styleUrls: ['./typing-animation.component.scss']
})
export class TypingAnimationComponent {
  private subs = new Subscription();
  expertiseSet:string[] =['Full Stack Developer','Financial Expert','Frontend Developer', 'Backend Developer'];
  expertiseSetRus:string[] =['Фулстек Разработчик','Финансовый Эксперт','Фронтенд Разработчик', 'Бэкенд Разработчик'];
  nextExpert:Subject<number> = new Subject();
  expertInd:number=0;
  expertDesc:string;
  cursor:string='|';
  timerTyping: ReturnType<typeof setTimeout> []=[];
  lang:languages=languages.ENGLISH;
  public readonly languages : typeof languages = languages;
  constructor(
    private comm : CommService
  ) {}
  ngOnInit(): void {
    this.subs.add(this.comm.langSub.subscribe(lang=>{
      this.lang=lang;
      if (this.cursor==='|') {
        this.toggleTyping()
        this.toggleTyping()
      } else {
      }
    }));
    this.typingSubscription()
    setTimeout(() => {this.nextExpert.next(this.expertInd)}, 200);
  }
  typingSubscription() {
    this.subs.add(
      this.nextExpert.subscribe(indExp=>{
      let expertiseSet = this.lang===languages.ENGLISH? this.expertiseSet:this.expertiseSetRus;
      let strLng:number = expertiseSet[indExp].length
      for (let index = 0; index <= strLng*2 ; index++) {
        this.timerTyping.push (setTimeout (()=>{
           index===strLng*2? this.nextExpert.next(this.expertInd=indExp===expertiseSet.length-1? 0: indExp+1):null;
           this.expertDesc = expertiseSet[indExp].slice(0, index + (index-strLng) *2* ~~(index/strLng)*-1  )
         },index<=strLng? index*180 :strLng*300 + (index-strLng)*100))
       } 
    }))
  }
  toggleTyping() {
    let expertiseSet = this.lang===languages.ENGLISH? this.expertiseSet:this.expertiseSetRus;
    if (this.cursor==='|') {
      this.timerTyping.forEach(i=>clearTimeout(i));
      this.expertDesc=expertiseSet[this.expertInd];
      this.cursor=''
    } else {
      this.nextExpert.next(this.expertInd=expertiseSet.length===this.expertInd+1? 0:this.expertInd+1)
      this.cursor='|'
    }
  }
  ngOnDestroy(): void {
    this.subs.unsubscribe();    
  }
}